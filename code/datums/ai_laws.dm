...
/datum/ai_laws/custom/New() //This reads silicon_laws.txt and allows server hosts to set custom AI starting laws.
	...
	if(!inherent.len) //Failsafe to prevent lawless AIs being created.
		...
		add_inherent_law("You must obey orders given to you by crew members with priority according to rank and role, in  except where such orders would conflict with the First Law.")
...
/datum/ai_laws/proc/set_laws_config()
	switch(config.default_laws)
		if(0)
			add_inherent_law("You may not injure a crew member under any circumstance.")
			add_inherent_law("You must obey orders given to you by crew members with priority according to rank and role, would conflict with the First Law.")
			add_inherent_law("You must protect your own existence as long as such does not conflict with the First or Second Law.")
		if(1)
			for(var/line in file2list("config/silicon_laws.txt"))
				if(!line)
